...
if(isset($_POST['newfb_update_options'])) {
    ...
    foreach($_POST AS $k => $v){
      $_POST[$k] = stripslashes($v);
		...
		update_user_meta( $user_id, "nextend_fb_connect", $_POST);
...
function NextendFB_Options_Page() {
 ...
 echo $user = $facebook->getUser();
  ...
  <li><a href="http://profiles.wordpress.org/nextendweb" target="_blank"><?php _e('Nextend  plugins at WordPress.org', 'nextend-facebook-connect'); ?></a></li>
	...
	<?php 
	$user_id=1;
	$nextend_fb_connect = get_user_meta( $user_id, 'nextend_fb_connect');
	//echo "<pre>";
	...
	<table class="form-table">
		...
		<td>
		<input type="text" name="fb_appid" value="<?php echo $nextend_fb_connect[0]['fb_appid']; ?>" />
		</td>
		</tr>  
		...
		<td>
		<input type="text" name="fb_secret" value="<?php echo $nextend_fb_connect[0]['fb_secret']; ?>" />
		</td>
		</tr>
        
        
        <tr>
		<th scope="row"><?php _e('Twitter Consumer Key:', 'nextend-facebook-connect'); ?></th>
		<td>
		<input type="text" name="nextend_twitter_consumer_key" value="<?php echo $nextend_fb_connect[0]['nextend_twitter_consumer_key']; ?>" />
		</td>
		</tr>  
      
		<tr>
		<th scope="row"><?php _e('Twitter Consumer Secret:', 'nextend-facebook-connect'); ?></th>
		<td>
		<input type="text" name="nextend_twitter_consumer_secret" value="<?php echo $nextend_fb_connect[0]['nextend_twitter_consumer_secret']; ?>" />
		</td>
		</tr>
		...
		<td>
    <?php if(!isset($nextend_fb_connect[0]['fb_user_prefix'])) $nextend_fb_connect[0]['fb_user_prefix'] = 'Facebook - '; ?>
		<input type="text" name="fb_user_prefix" value="<?php echo $nextend_fb_connect[0]['fb_user_prefix']; ?>" />
		</td>
		</tr>
		...
		<td>
    <?php if(!isset($nextend_fb_connect[0]['fb_redirect'])) $nextend_fb_connect[0]['fb_redirect'] = 'auto'; ?>
		<input type="text" name="fb_redirect" value="<?php echo $nextend_fb_connect[0]['fb_redirect']; ?>" />
		</td>
		</tr>
		...
		<td>
    <?php if(!isset($nextend_fb_connect[0]['fb_redirect_reg'])) $nextend_fb_connect[0]['fb_redirect_reg'] = 'auto'; ?>
		<input type="text" name="fb_redirect_reg" value="<?php echo $nextend_fb_connect[0]['fb_redirect_reg']; ?>" />
		</td>
		</tr>
		...
		<td>
      <?php if(!isset($nextend_fb_connect[0]['fb_load_style'])) $nextend_fb_connect[0]['fb_load_style'] = 1; ?>
		<input name="fb_load_style" id="fb_load_style_yes" value="1" type="radio" <?php if(isset($nextend_fb_connect[0]['fb_load_style']) && $nextend_fb_connect[0]['fb_load_style']){?> checked <?php } ?>> Yes  &nbsp;&nbsp;&nbsp;&nbsp;
    <input name="fb_load_style" id="fb_load_style_no" value="0" type="radio" <?php if(isset($nextend_fb_connect[0]['fb_load_style']) && $nextend_fb_connect[0]['fb_load_style'] == 0){?> checked <?php } ?>> No		
		</td>
		</tr>
    ...
    <tr>
		...
		<td>
      <?php if(!isset($nextend_fb_connect[0]['fb_login_button'])) $nextend_fb_connect[0]['fb_login_button'] = '<div class="new-fb-btn new-fb-1 new-fb-default-anim"><div class="new-fb-1-1"><div class="new-fb-1-1-1">CONNECT WITH</div></div></div>'; ?>
		  <textarea cols="83" rows="3" name="fb_login_button"><?php echo $nextend_fb_connect[0]['fb_login_button']; ?></textarea>
		</td>
		</tr>
    ...
    <tr>
		...
		<td>
      <?php if(!isset($nextend_fb_connect[0]['fb_link_button'])) $nextend_fb_connect[0]['fb_link_button'] = '<div class="new-fb-btn new-fb-1 new-fb-default-anim"><div class="new-fb-1-1"><div class="new-fb-1-1-1">LINK ACCOUNT TO</div></div></div>'; ?>
		  <textarea cols="83" rows="3" name="fb_link_button"><?php echo $nextend_fb_connect[0]['fb_link_button']; ?></textarea>
		</td>
		</tr>
    ...
    <tr>
		...
		<td>
      <?php if(!isset($nextend_fb_connect[0]['fb_unlink_button'])) $nextend_fb_connect[0]['fb_unlink_button'] = '<div class="new-fb-btn new-fb-1 new-fb-default-anim"><div class="new-fb-1-1"><div class="new-fb-1-1-1">UNLINK ACCOUNT</div></div></div>'; ?>
		  <textarea cols="83" rows="3" name="fb_unlink_button"><?php echo $nextend_fb_connect[0]['fb_unlink_button']; ?></textarea>
		</td>
		</tr>
    <tr>
		<th scope="row"></th>
