...
$new_fb_settings = get_user_meta( $user_id, 'nextend_fb_connect');
...
if (!isset($new_fb_settings[0]['fb_load_style'])) $new_fb_settings[0]['fb_load_style'] = 1;
if ($new_fb_settings[0]['fb_load_style']) {
...
function new_twitter_login_action() {
  ...
  if (isset($_GET['action']) && $_GET['action'] == 'unlink') {
    ...
    if ($user_info->ID) {
      delete_user_meta( $user_info->ID, "nextend_twitter_token_key");
	  delete_user_meta( $user_info->ID, "nextend_twitter_secret_key");
...
function new_fb_login_action() {
  ...
  if (isset($_GET['action']) && $_GET['action'] == 'unlink') {
    $user_info = wp_get_current_user();
    if ($user_info->ID) {
      $wpdb->query($wpdb->prepare('DELETE FROM wp_social_users
          WHERE ID = %d
  ...
  if ($user && is_user_logged_in() && new_fb_is_user_connected()) {
    ...
    try {
      ...
      $ID = $wpdb->get_var($wpdb->prepare('
        SELECT ID FROM wp_social_users WHERE type = "fb" AND identifier = "%d"
      ', $user_profile['id']));
      if (!get_user_by('id', $ID)) {
        $wpdb->query($wpdb->prepare('
          DELETE FROM wp_social_users WHERE ID = "%d"
        ', $ID));
        $ID = null;
      }
      if (!is_user_logged_in()) {
        if ($ID == NULL) { // Register
          ...
          if ($ID == false) { // Real register
            ...
            if (!isset($new_fb_settings[0]['fb_user_prefix'])) $new_fb_settings[0]['fb_user_prefix'] = 'facebook-';
            $sanitized_user_login = sanitize_user($new_fb_settings[0]['fb_user_prefix'] . $user_profile['username']);
          ...
          if ($ID) {
            $wpdb->insert('wp_social_users', array(
              'ID' => $ID,
              'type' => 'fb',
          ...
          if (isset($new_fb_settings[0]['fb_redirect_reg']) && $new_fb_settings[0]['fb_redirect_reg'] != '' && $new_fb_settings[0]['fb_redirect_reg'] != 'auto') {
            set_site_transient( nextend_uniqid().'_fb_r', $new_twitter_settings[0]['twitter_redirect_reg'], 3600);
          }
        }
        if ($ID) { // Login
    ...
    $scope = apply_filters('nextend_fb_scope', 'email,publish_stream');
    $loginUrl = $facebook->getLoginUrl(array(
      'scope' => $scope
    ));
    if (isset($new_fb_settings[0]['fb_redirect']) && $new_fb_settings[0]['fb_redirect'] != '' && $new_fb_settings[0]['fb_redirect'] != 'auto') {
      $_GET['redirect'] = $new_fb_settings[0]['fb_redirect'];
    }
...
function new_fb_is_user_connected() {
  ...
  $current_user = wp_get_current_user();
  $ID = $wpdb->get_var($wpdb->prepare('
    SELECT identifier FROM wp_social_users WHERE type = "fb" AND ID = "%d"
  ', $current_user->ID));
...
function new_fb_sign_button() {
  ...
  global $new_fb_settings;
  return '<a href="' . new_fb_login_url() . (isset($_GET['redirect_to']) ? '&redirect=' . $_GET['redirect_to'] : '') . '" rel="nofollow">' . $new_fb_settings[0]['fb_login_button'] . '</a><br />';
}

function new_fb_link_button() {

  global $new_fb_settings;
  return '<a href="' . new_fb_login_url() . '&redirect=' . new_fb_curPageURL() . '">' . $new_fb_settings[0]['fb_link_button'] . '</a><br />';
}

function new_fb_unlink_button() {

  global $new_fb_settings;
  return '<a href="' . new_fb_login_url() . '&action=unlink&redirect=' . new_fb_curPageURL() . '">' . $new_fb_settings[0]['fb_unlink_button'] . '</a><br />';
}
...
//add_action( 'save_post', '_save_post_fb_post', 10, 2 );
//add_action ( 'rtmedia_before_media_gallery', '_save_post_fb_post' );
function _save_post_fb_post($post_id)
{
		global $wpdb;
		...
		if ($user_info->ID) {
				...
				if (isset($_POST) && isset($target_id->identifier)) {
					 ...
					 $post_id = $facebook->api(
							...
							array(
								'method' => 'stream.publish',
								'message' => "Hello dear Please this post",
								//'attachment' => "",
								//'action_links' => ,
								//'target_id' => $target_id,
...
function post_check_shortcode( $atts ) 
{
		global $wpdb;
		require_once (dirname(__FILE__) . '/sdk/init.php');
		$target_id = $facebook->getUser();
 		
		return $post_id = $facebook->api(
                //See stream publish documentation on developers.facebook.com
                //to see more parameters
                array(
                    'method' => 'stream.publish',
                    'message' => "hello my dearm comment here 2nd testdd",
                    //'attachment' => "",
                    //'action_links' => ,
                    //'target_id' => $target_id,
                    'uid' => $target_id
...
add_shortcode( 'post-check', 'post_check_shortcode' );

function new_add_fb_twitter_login_form() {
  ...
  jQuery(document).ready(function(){
    (function($) {
      ...
      if(!window.twitter_added){
        socialLogins.prepend('<?php echo addslashes(preg_replace('/^\s+|\n|\r|\s+$/m', '', new_fb_twitter_sign_button())); ?>');
...
//add_action('login_form', 'new_add_fb_twitter_login_form');
function new_fb_twitter_link_button() {
	...
	// Ask twitter for a request token and specify a callback url upon which twitter will redirect back to this url after authorization of app.
	
	try {
		$url 		= site_url('/nextend-twitter-kyes/step2.php');
		$Token 		= $Client->get_oauth_request_token($url);
	...
	update_user_meta(1, 'nextend_token_secret_connect', $Token->secret);
	 $new_twitter_settings['twitter_login_button'] = '<div class="new-twitter-btn new-twitter-1 new-twitter-default-anim"><div class="new-twitter-1-1"><div class="new-twitter-1-1-1">LINK ACCOUNT TO</div></div></div>';
	  return '<a href="'.$redirect.'" rel="nofollow">' . $new_twitter_settings['twitter_login_button'] . '</a><br />';
	}
}
...
function new_fb_twitter_unlink_button() {

  return '<a href="' . new_twitter_login_url() . '&action=unlink&redirect=' . new_fb_curPageURL() . '"><div class="new-twitter-btn new-twitter-1 new-twitter-default-anim"><div class="new-twitter-1-1"><div class="new-twitter-1-1-1">UNLINK ACCOUNT</div></div></div></a><br />';
}
...
function new_add_fb_twitter_connect_field() {
          ...
          <?php
			 /* if (new_twitter_is_user_connected()) {
				...
				if ($user_info->ID) {
							...
							if(isset($nextend_twitter_token_key[0]) && $nextend_twitter_token_key[0]!='')
							{
								echo new_fb_twitter_unlink_button();
							}
							else
							{
								echo new_fb_twitter_link_button();
							}
...
add_action('profile_personal_options', 'new_add_fb_twitter_connect_field');
